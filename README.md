...
Initially I had styled the footer in the same way as the navbar but decided to change it for that it stands out better against the background when scrolling. I chose to use the sticky property on the footer as it creates a positive UX experience. The icons are generated by Font Awesome and all have relevant aria-labels to ensure accessibility for all users.
