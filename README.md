...
Initially I planned to just have a hero image on the landing page with a welcome message. Users would of had to use the navigation menu to access any other parts of the site. The revised version is much more user friendly, as the landing page now features the about section and includes links to other pages directly from the content.

![Home page wireframe](assets/images/wireframes/home-page.png)

**Classes Page**

As previosuly mentioned, in the first draft I intended to have a landing page for all the classes and then a dedicated page for each class. The design was then streamlined to incorporate one classes page with short descriptions for each class. The result is a consise class page that works in its own right but could be added to in the future.

![Classes page wireframe](assets/images/wireframes/class-page.png)

**Gallery Page**

Initially I had planned to use Bootstrap to create the gallery which would of enabled the use of Javascript and provided more options with interactitvity. However as I moved away from Bootstrap I needed to find another solution and used CSS grids to create a static gallery.
...
I chose to use a hero image on each page to keep the format consistent across the site. The exception is the gallery page, as I felt including a hero image would be unneccessary and possibly interfere with the user experience as it would be too cluttered.

I chose the hero images for a number of reasons. On the home page, I wanted it to feel immersive as if you were taking part in a class. On the classes page, I chose to use a picture of a slightly out of focus studio, again to create a sense of being there. On the contact page I chose a cactus to provide a pop of colour and to be slightly playful, the idea of a cactus being approachable is a play on assumptions.

All images used in the site are free stock photos, citation is available in the [credits](#credits) section.
...
The navigation menu consists of four relevant pages and is built with Bootstrap to enable responsiveness when viewed on small screens. I used Bootstrap's `navbar-brand` in place of a logo, which could be changed at a later date if I was to develop this site further. I used CSS to apply a darker font and make the `hover` effect slightly darker as the default did not provide enough contrast.

![Navbar](/assets/images/readme-images/navbar.png)

**Footer**

Initially I had styled the footer in the same way as the navbar but decided to make it darker so that it stands out better against the background when scrolling. I chose to use the sticky property on the footer as it creates a positive UX experience and ensures users see it. The icons are generated by Font Awesome and all have relevant aria-labels to ensure accessibility for all users.
...
Finally, I added a 404 page in case of errors. I chose a picture of a broken plate as it is quite humorous and relates back to the theme of pottery. I used Canva to adjust the colours to make the image more playful.
